...
public class ImapSourceFunction extends RichSourceFunction<RowData> {
    ...
    // TODO Allow force-disabling this?
    ...
    private void collectMessage(SourceContext<RowData> ctx, Message message) throws MessagingException {
        ...
        for (int i = 0; i < fieldNames.length; i++) {
            switch (fieldNames[i].toUpperCase()) {
                ...
                case "HEADERS":
                    // TODO Prettify
                    row.setField(i, new GenericArrayData(Collections.list(message.getAllHeaders()).stream()
                        .map(header -> GenericRowData.of(StringData.fromString(header.getName()), StringData.fromString(header.getValue())))
                        .toArray()));
                    break;
                case "FROM":
    ...
    private ArrayData mapAddressItems(Address[] items) {
        ...
        var mappedItems = Arrays.stream(items)
